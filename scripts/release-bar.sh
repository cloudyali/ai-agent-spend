#!/usr/bin/env bash
# release-bar.sh — build the aispend-bar macOS menu-bar app into a distributable .dmg.
#
...
DMG="$DIST/aispend-bar-$VERSION.dmg"
...
# 1. Universal binary (Apple Silicon + Intel) via lipo. The macOS-only menuet dep is
#    fetched by the caller (CI does `go get github.com/caseymrm/menuet` first).
...
# 2. Info.plist — LSUIElement=1 makes it a menu-bar agent (no Dock icon).
...
# 3. Sign. Developer ID + hardened runtime when APPLE_SIGN_IDENTITY is set (Phase 2);
...
hdiutil create -volname "aispend-bar" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
rm -rf "$STAGE"

# 5. Notarize + staple (Phase 2) — only when the notary credentials are present.
...
# 6. Checksum for the Homebrew cask.

...
// It defaults to 5 minutes and accepts any Go duration string (15m, 30m, 1h, 90s).
...
func TestLoadScanInterval(t *testing.T) {
	cases := []struct {
		...
		{"absent file defaults to 5m", "", DefaultScanInterval, false},
		{"blank value defaults to 5m", "scan_interval =\n", DefaultScanInterval, false},
		...
		{"missing unit errors, defaults 5m", "scan_interval = 15\n", DefaultScanInterval, true},
		{"garbage errors, defaults 5m", "scan_interval = soon\n", DefaultScanInterval, true},
		{"zero rejected, defaults 5m", "scan_interval = 0\n", DefaultScanInterval, true},
